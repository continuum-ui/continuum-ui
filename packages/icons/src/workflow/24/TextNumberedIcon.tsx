import * as React from "react";
import { iconVariants } from "@continuum-ui/icons/helpers";
import type { IconProps } from "@continuum-ui/icons/helpers";

export const TextNumberedIcon = React.forwardRef<SVGSVGElement, IconProps>(
    ({ className, size, focusable = false, hidden = true, color = "currentColor", ...props }, forwardedRef) => {
        return (
            <svg
                xmlns="http://www.w3.org/2000/svg"
                height="48"
                viewBox="0 0 48 48"
                width="48"
                className={iconVariants({ size, className })}
                focusable={focusable}
                aria-hidden={hidden}
                {...props}
                ref={forwardedRef}
            >
                <path d="M6.173 38.857c-.092 0-.128-.037-.128-.128v-1.82c0-.11 0-.183.11-.183l.916-.009c1.287 0 1.985-.385 1.985-1.231 0-.808-.68-1.341-2.022-1.341a5.657 5.657 0 0 0-2.719.7c-.11.055-.128 0-.128-.073V32.95c0-.11-.019-.147.092-.2a6.783 6.783 0 0 1 3.2-.717c2.426 0 3.933 1.213 3.933 3.124a2.605 2.605 0 0 1-1.618 2.408 2.918 2.918 0 0 1 2.188 2.867c0 2.352-2.169 3.6-4.7 3.6a6.625 6.625 0 0 1-3.143-.606c-.11-.037-.11-.147-.11-.239V41.2c0-.073.092-.11.166-.073a6.269 6.269 0 0 0 3 .772c1.654 0 2.3-.68 2.3-1.544 0-.974-.7-1.507-2.225-1.507ZM6.785 2.371a15.522 15.522 0 0 1-1.953.507c-.127.018-.163-.018-.163-.127V1.177c0-.091.018-.145.126-.163A11.617 11.617 0 0 0 7.134.091.661.661 0 0 1 7.442 0h1.479c.09 0 .108.054.108.127v9.691h1.618c.127 0 .163.054.181.163l.005 1.82c.018.145-.036.2-.145.2H5.053c-.127 0-.163-.054-.145-.163V9.982a.172.172 0 0 1 .2-.163H6.78ZM3.817 28c-.126 0-.144-.054-.144-.162v-1.292a.256.256 0 0 1 .09-.233 44.009 44.009 0 0 0 3.374-3.033C8.555 21.9 9.172 21.005 9.172 20c0-1.131-.922-1.792-2.286-1.792A6.455 6.455 0 0 0 4 19c-.107.054-.179.018-.179-.107v-1.78a.206.206 0 0 1 .107-.216A6.847 6.847 0 0 1 7.478 16c2.638 0 3.887 1.571 4 3.578a5.289 5.289 0 0 1-1.854 4.095 27.85 27.85 0 0 1-2.276 2.2c1.238 0 3.789-.033 4.848-.033.126 0 .144.035.126.161l-.535 1.858a.178.178 0 0 1-.2.144Z" />
                <rect height="4" rx="1" ry="1" width="26" x="18" y="8" />
                <rect height="4" rx="1" ry="1" width="26" x="18" y="24" />
                <rect height="4" rx="1" ry="1" width="26" x="18" y="40" />
            </svg>
        );
    },
);
